...
def train_epoch(train_loader, model, loss_fn, optimizer, cuda, log_interval, metrics, writer, writer_train_index):
    ...
    for batch_idx, (anchor, pairs, membership) in enumerate(train_loader):
        #this is where the cuda stuff was

        optimizer.zero_grad()
        embedded_anchor = model(anchor)
        embedded_pairs = model(pairs)
        
        loss_outputs = loss_fn.apply(embedded_anchor, embedded_pairs, membership)
        ...
        if batch_idx % log_interval == 0:
            ...
            message = 'Train: [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                batch_idx * len(anchor), len(train_loader.dataset),
...
def test_epoch(val_loader, model, loss_fn, cuda, metrics):
    with torch.no_grad():
        ...
        model.eval()
        val_loss = 0
        for batch_idx, (anchor, pairs, membership) in enumerate(val_loader):
